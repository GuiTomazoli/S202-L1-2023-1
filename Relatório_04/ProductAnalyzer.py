...
class Mercado:
    ...
    def totalvendas(self): #Lista o total de vendas por dia do mercado.
        return self.db.collection.aggregate([
            {"$unwind": "$produtos"},
            {"$group": {"_id": {"data": "$data_compra"}, "total": {"$sum": {"$multiply": ["$produtos.quantidade", "$produtos.preco"]}}}},
            {"$sort": {"_id.data": 1, "total": -1}},
])
    def produtomaisvendido(self): #Lista o produto mais vendido em todas as compras do mercado.
        return self.db.collection.aggregate([
            {"$unwind": "$produtos"},
    ...
    def gastouunicacompra(self): #Lista o cliente que mais gastou em uma única compra do mercado.
        return self.db.collection.aggregate([
            {"$unwind": "$produtos"},
    ...
    def quantidadeacimadeum(self): #Lista todos os produtos que tiveram uma quantidade vendida acima de 1 unidades.
        return self.db.collection.aggregate([
            {"$unwind": "$produtos"},
